...
24/12/2024 update:
拖了这么久，终于有时间写了，后续会实现一个c++版本的。下面有详细说明，如何伪装成slave，如何拉取binlog，如何应用十六进制数据。这个版本默认主从网络延迟为0
```
...
```bash
伪装成一个slave，从master拿到binlog的stream流文件，如果没有触发切换，就不断地舍弃掉前面获取的binlog，如果orchestrator触发了切换，则拿到对应Exec_Master_Log_Pos后续的日志内容。到新主库上应用
```

如何伪装？
```bash
实现了一个mysql协议，去连接到master，持续地获取binlog日志。假设网络延迟为0。
通常情况下，mysql主从复制的延迟点在于slave的io线程写入磁盘，sql线程应用日志产生的延迟
工具避免了大量写入磁盘操作，去除了binlog中大量的header信息和其他信息，极大缩小了写入磁盘的大小。缩小比例大约100:1
程序会动态获取当前延迟信息，从而进行日志保留的操作：
1、在没有延迟的情况下，默认保留5分钟前的日志
2、在延迟的情况下，保留时间为：延迟时间+5Min前的日志
```

如何实现数据应用？
```bash
通过拿到binlig的stream流数据，解析到十六进制格式的内容，通过结构体转换成对应的字段。重新实现一遍各类型的从十六进制存储到前端显示的数据
```
